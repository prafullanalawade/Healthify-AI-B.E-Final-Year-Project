...
def heart_visualization_page():
    ...
    column_mapping = {
        ...
        "Slope of Peak Exercise ST Segment": "Slope of Peak Exercise ST Segment",
        "Major Vessels Colored by Fluoroscopy": "Major Vessels Colored by Fluoroscopy",
    ...
    ax.set_title('Heart Disease Prediction Input Features', fontsize=14, pad=20)
    ...
    heart_result = st.session_state.get("heart_result", "No Prediction Made")

    if heart_result in ["The person may have heart disease", "The person does not have heart disease"]:
        st.subheader("Prediction Result Distribution")
        has_disease = 1 if heart_result == "The person may have heart disease" else 0

        result_counts = {
            "Has Heart Disease": has_disease,
            "No Heart Disease": 1 - has_disease
        }
        ...
        ax.set_title("Heart Disease Prediction Result")
        st.pyplot(fig)
    else:
